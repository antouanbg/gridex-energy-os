...
проверката отвън остава. Frontend branch commit `4e514aa` е локален: push-ът
бе отказан от автоматичната проверка до отделно изрично потвърждение; не е
публикуван към main и отказът не се заобикаля. Изходният код на docs сайта е
в отделен локален Git repo `gridex-docs` (без remote). Първа реална клиентска
