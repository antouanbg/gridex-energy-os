...
are still pending; do not deploy/claim production login before those checks.
...
промяна. Public issuer на backend е проверен, но точните Keycloak callbacks и
реалният browser тест с обикновен user предстоят; не публикувай/заявявай
production login преди тези проверки.
