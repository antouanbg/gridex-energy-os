...
function FlexibleLoads({notify,lang}:{notify:(v:string)=>void;lang:UiLanguage}) {
  const t=(bg:string,en:string)=>lang==="en"?en:bg;
  const [tab,setTab]=useState<"assets"|"rules"|"thermal">("assets");
  ...
  return <div className="loads-page" data-no-translate>
    ...
    <div className="subnav loads-tabs">{[["assets",t("Активи и график","Assets & schedule")],["rules",t("Правила за покупка","Purchase rules")],["thermal",t("Термичен модел","Thermal model")]].map(x=><button key={x[0]} className={tab===x[0]?"active":""} onClick={()=>setTab(x[0] as typeof tab)}>{x[1]}</button>)}</div>
...
function About({lang,notify}:{lang:UiLanguage;notify:(v:string)=>void}) {
  ...
  return <div className="about-page" data-no-translate>
    ...
    <section className="card developer-card">
      ...
      <div className="developer-copy"><p>{t("АРХИТЕКТ И РАЗРАБОТЧИК НА СИСТЕМАТА","SYSTEM ARCHITECT & DEVELOPER")}</p><h2>Antouan Anguelov</h2><span>{t("Автор на концепцията и архитектурата на GrideX Energy OS — от OpenRemote asset модела и локалния Modbus концентратор до пазарната оптимизация, прогнозите и защитната логика за PV, BESS и управляеми консуматори.","Creator of the GrideX Energy OS concept and architecture — from the OpenRemote asset model and local Modbus gateway to market optimisation, forecasting and safety logic for PV, BESS and controllable loads.")}</span><div className="developer-links"><a href="https://linkmy.cards/en/antouan-anguelov/" target="_blank" rel="noreferrer">Digital profile ↗</a><a href="https://www.linkedin.com/in/antouan/" target="_blank" rel="noreferrer">LinkedIn ↗</a></div></div>
