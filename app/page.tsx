...
type EdgeGatewayDraft = { id:string; hardware:"tcan485"|"waveshare"; deviceType:string; modelId:string; channel:string; address:string; stage:EdgeGatewayStage };
...
function EdgeHardwareConfigurator({lang,notify}:{lang:UiLanguage;notify:(v:string)=>void}) {
  ...
  const hardware={
    tcan485:{label:"LilyGo T-CAN485",channels:["CAN","RS485"]},
    waveshare:{label:"Waveshare 2-CH CAN To Ethernet",channels:["CAN 1","CAN 2","RS485"]},
  };
  const [gateways,setGateways]=useState<EdgeGatewayDraft[]>([
    {id:"GW-01",hardware:"tcan485",deviceType:"inverter",modelId:"deye-modbus",channel:"RS485",address:"1",stage:"draft"},
    {id:"GW-02",hardware:"waveshare",deviceType:"battery",modelId:"pylontech-can",channel:"CAN 1",address:"0x351",stage:"draft"},
  ]);
  ...
  const addGateway=()=>setGateways(current=>{const number=Math.max(0,...current.map(item=>Number(item.id.replace("GW-",""))))+1;return [...current,{id:`GW-${String(number).padStart(2,"0")}`,hardware:"tcan485",deviceType:"meter",modelId:"eastron-meter",channel:"RS485",address:"1",stage:"draft"}];});
  ...
  return <section className="edge-configurator" data-no-translate>
    ...
    <div className="gateway-config-list">{gateways.map(gateway=>{const model=modelFor(gateway);const channels=compatibleChannels(gateway,model);const target=gateway.hardware==="tcan485"?t("ESP32 firmware на нода","ESP32 node firmware"):t("Driver service на ROCK Pi E","ROCK Pi E driver service");return <article className="card gateway-config-card" key={gateway.id}>
      ...
      <div className="gateway-config-fields">
        <label><span>{t("Хардуер на гейта","Gateway hardware")}</span><select value={gateway.hardware} onChange={e=>updateGateway(gateway.id,{hardware:e.target.value as EdgeGatewayDraft["hardware"]})}><option value="tcan485">{hardware.tcan485.label}</option><option value="waveshare">{hardware.waveshare.label}</option></select></label>
...
function SupportedEdgeHardware({lang,notify}:{lang:UiLanguage;notify:(v:string)=>void}) {
  ...
  const hardware=[
    ...
    {kind:t("ИНДУСТРИАЛЕН CAN/ETH ШЛЮЗ","INDUSTRIAL CAN/ETH GATEWAY"),icon:"W",name:"Waveshare 2-CH CAN TO ETH",status:t("Поддържан транспорт","Supported transport"),summary:t("Готов индустриален transport за свързване на CAN и RS485 устройства към OT LAN.","Ready industrial transport for connecting CAN and RS485 devices to the OT LAN."),features:["2× CAN 2.0B + 1× RS485 + Ethernet",t("CAN repeater и CAN → RS485","CAN repeater and CAN → RS485"),"Dual SOCKET · TCP/UDP",t("Изолация, metal case, −40…+85°C","Isolation, metal case, −40…+85°C")]},
    {kind:t("ПРОГРАМИРУЕМ DEVICE NODE","PROGRAMMABLE DEVICE NODE"),icon:"T",name:"LilyGo T-CAN485",status:t("Нод по устройство","Per-device node"),summary:t("Отделен ESP32 нод с конкретен драйвер за инвертор, BMS, електромер или EVSE.","A separate ESP32 node with a specific driver for an inverter, BMS, meter or EVSE."),features:["ESP32 · 4 MB Flash",t("1× CAN + 1× RS485","1× CAN + 1× RS485"),t("Локален driver и унифицирана MBUS карта","Local driver and unified MBUS map"),t("UID, auto-discovery и дистанционна конфигурация","UID, auto-discovery and remote configuration")]},
  ];
  return <article className="card edge-hardware-suite" data-no-translate><div className="edge-hardware-title"><div><p>{t("ПОДДЪРЖАНО ЦЯЛОСТНО РЕШЕНИЕ","SUPPORTED END-TO-END SOLUTION")}</p><h2>{t("Три отделни хардуерни роли в GrideX Edge","Three distinct hardware roles in GrideX Edge")}</h2><span>{t("ROCK Pi E управлява; Waveshare пренася индустриалните CAN/RS485 канали; T-CAN485 изпълнява конкретния device driver до оборудването.","ROCK Pi E controls; Waveshare transports industrial CAN/RS485 channels; T-CAN485 runs the device-specific driver next to the equipment.")}</span></div><div className="hardware-role-flow"><b>ROCK Pi E</b><i>↔</i><b>Waveshare</b><i>↔</i><b>T-CAN485</b></div></div><div className="edge-hardware-cards">{hardware.map(item=><section key={item.name}><header><i>{item.icon}</i><span><small>{item.kind}</small><strong>{item.name}</strong></span><em>{item.status}</em></header><p>{item.summary}</p><ul>{item.features.map(feature=><li key={feature}>✓ {feature}</li>)}</ul></section>)}</div><button className="primary-btn edge-hardware-action" onClick={()=>notify(t("Цялостният Edge хардуерен профил е избран","The complete Edge hardware profile is selected"))}>{t("Избери цялостното решение","Select the complete solution")}</button></article>;
}
