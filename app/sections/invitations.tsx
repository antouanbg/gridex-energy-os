...
export function Invitations({ api, lang }: { api: GridexApiClient; lang: UiLanguage }) {
  const t = copy[lang];
  ...
  return <section className="card config-card" data-no-translate aria-label={t.title}>
    <h2>{t.title}</h2>
    ...
    {!loading && <>
      ...
      {!admins.length ? <p>{t.noadmin}</p> : <form onSubmit={event => {
