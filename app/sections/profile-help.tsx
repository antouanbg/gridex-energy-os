...
export function ProfileHelp({lang,live}:{lang:UiLanguage;live:boolean}) {
  ...
  return <div className="profile-help-page" data-no-translate>
    ...
    <div className="profile-help-grid">
      ...
      <article id="email-notifications" className="card profile-help-topic"><span>03 / {t('ИЗВЕСТИЯ','NOTIFICATIONS')}</span><h3>{t('Какво прави отметката','What the checkbox does')}</h3><p>{t('„Получавай имейл за всички бъдещи събития“ е Ваш постоянен избор, изключен по подразбиране. Включването не изпраща писма за минали събития и не изисква одобрение за всяко следващо. Засега се следи загуба на връзка с ROCK Pi и последният успешен контакт с ESP32 през ROCK Pi: един имейл за устройство и прекъсване, нов само след възстановяване и ново прекъсване. Ако ROCK Pi спре, липсата на нови данни от ESP32 сама по себе си не изпраща втори имейл. Следващите видове събития ще използват същия избор, след като бъдат внедрени.','“Email me about all future events” is your persistent preference, off by default. Enabling it does not send messages for past events or require approval for each future event. Currently, lost ROCK Pi contact and the last successful ESP32 contact through ROCK Pi are monitored: one email per device and outage, and another only after recovery and a new outage. When ROCK Pi stops, the resulting lack of ESP32 data alone does not trigger a duplicate email. Future event types will use the same choice once implemented.')}</p></article>
