...
export function DeviceAccess({api,siteId,lang}:{api:GridexApiClient;siteId:string;lang:UiLanguage}) {
 const t=(bg:string,en:string)=>lang==='en'?en:bg;
 const [gateways,setGateways]=useState<{id:string;name:string;role:string}[]>([]);
 const [gateway,setGateway]=useState('');const [version,setVersion]=useState<number|null>(null);
 ...
 <label>{t('Контролер','Controller')}<select required value={gateway} onChange={e=>{setVersion(null);setGateway(e.target.value);}}><option value="">—</option>{gateways.map(g=><option key={g.id} value={g.id}>{g.name}</option>)}</select></label>
